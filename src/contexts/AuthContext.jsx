...
export function AuthProvider({ children }) {
  ...
  // Listen for auth state changes AND user document changes
  useEffect(() => {
    let unsubscribeUserDoc = null;
    const unsubscribeAuth = authService.subscribeToAuthChanges(async (firebaseUser) => {
      if (firebaseUser) {
        // User is logged in, listen to their Firestore document for real-time updates
        ...
        unsubscribeUserDoc = onSnapshot(userRef, (userDoc) => {
          if (userDoc.exists()) {
            ...
            // If user document is deleted, treat as logged out
            setCurrentUser(null);
          }
          setLoading(false);
        }, (error) => {
          // Handle errors listening to user doc
        ...
        // User is logged out
        if (unsubscribeUserDoc) unsubscribeUserDoc(); // Stop listening to previous user doc
        setCurrentUser(null);
        setLoading(false);
      }
    });

    // Cleanup function: unsubscribe from auth and user doc listeners
    return () => {
      unsubscribeAuth();
